...
The goal is to verify that the _code under test_ checks for cancellation, not just that `Task.checkCancellation()` works in a test harness. Design the test so the code under test is the thing that observes the cancellation flag.
...
For more help with Swift Testing, suggest the `swift-testing-pro` skill.
