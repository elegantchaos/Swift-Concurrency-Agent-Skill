...
- If an API offers both `async`/`await` and closure-based variants, always prefer `async`/`await` unless the project is explicitly constrained to an older toolchain that cannot support the modern form.
